...
void expr_codegen(struct expr *e) {
  ...
  switch (e->kind) {
    ...
    //  this will ignore the higher-order bits in the result
    case EXPR_MUL:
      expr_codegen(e->left);
      expr_codegen(e->right);
    ...
    // See page 156-157 for a discussion of using idiv.
    ...
    case EXPR_INCREMENT:
      expr_codegen(e->left);
      fprintf(ASM_CODE, "ADDQ $1, %s\n", scratch_name(e->left->reg));
      e->reg = e->left->reg;
      break;
    
    case EXPR_DECREMENT:
      expr_codegen(e->left);
      fprintf(ASM_CODE, "SUBQ $1, %s\n", scratch_name(e->left->reg));
      e->reg = e->left->reg;
      break;
...
void stmt_codegen(struct stmt *s) {
  ...
  switch (s->kind) {
    ...
    case STMT_IF_ELSE: ;
      int e_label = label_create();
      int complete = label_create();
      expr_codegen(s->expr);
